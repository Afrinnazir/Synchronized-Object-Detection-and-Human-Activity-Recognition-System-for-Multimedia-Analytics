    ...
    if results_pose.pose_landmarks is not None:
        ...
        if len(frame_buffer) == buffer_size:
            ...
            if isinstance(results_object, list):
                ...
                detected_objects = [object_labels[int(obj[5])] for obj in results_object]

            # Combine activity and objects into one sentence
            if predicted_activity != prev_activity or set(detected_objects) != set(prev_objects):
                objects_sentence = ", ".join(detected_objects)
                output_sentence = f"The person is {predicted_activity.lower()} in {objects_sentence}"
                print(output_sentence)  # Print output sentence
                prev_activity = predicted_activity
                prev_objects = detected_objects

            # Display combined sentence on the frame
            cv2.putText(frame, output_sentence, (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
